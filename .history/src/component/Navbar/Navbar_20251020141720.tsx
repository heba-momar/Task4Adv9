...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`navbar p-x  ${isMenuOpen ? 'navbar-open' : ''} ${isScrolled ? 'scrolled' : ''}`}>
        ...
        <button className="nav-btn mobile-login">Login</button>

        <button className="burger" onClick={toggleMenu}>
          <img src={isMenuOpen ? CloseIcon : banner} alt="Menu Icon" />
        </button>
        <button className="nav-btn desktop-only">login</button>
      </nav>
    </>
  );
}
